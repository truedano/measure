...
### 6. 多國語言與語系切換功能 (Multi-Language & i18n Switcher)
- **規格**：系統支援**繁體中文 (zh)**與**英文 (en)**雙語系，且預設語系為繁體中文：
  - **切換按鈕**：在頂部工具列最右端新增一個齒輪設定圖示，點擊後展開下拉選單供切換語言。
  - **無縫即時變更**：切換語言時，全域元件的所有文字與說明標籤（包括工具列、側邊欄、Canvas 測量面板、試算表、自訂對話框按鈕與 Toast 提示）應立即切換語系，無須重新載入網頁。
  - **語系持久化**：使用者的語系設定需儲存於 `localStorage` 中，確保每次重新整理或重新開啟網頁時保持原有的語系選擇。
  - **開發代碼規範**：未來所有新增、修改或刪除 UI 顯示文字（包括標籤、預設文字、工具提示、彈窗內容等）的變更，皆必須將對應文字定義在 `workspaceStore.ts` 的 `locales` 字典中，並透過 `store.t()` 進行動態多國語言翻譯，嚴禁在元件中寫死字串字面量。

### 7. 繪圖操作引導與輔助工具 (Drawing Guides & Assistive Tools)
- **規格**：優化畫布上的操作引導與精確控制：
  - **動態滑鼠氣泡提示 (Cursor Tooltips)**：當處於繪圖準備或拉伸狀態時，滑鼠游標旁會渲染一個絕對定位的小型提示盒（以 `isHoveringOverlay` 及 `trackMouse` 排除 Handles 避免衝突）。提示文字需套用多國語言，並根據繪製模式顯示對應的指引與 Shift 鎖定提示。參考線模式提示需以極光紫呈現，測量線模式則為霓虹藍。
  - **正交角度鎖定輔助 (Orthogonal Angle Snapping)**：拉伸線段（包括繪製時或拖曳控制點時），若使用者按住鍵盤 `Shift` 鍵，夾角會被限制在最接近的 0°、45° 或 90° 方向上（相對於起點或對向端點）。
  - **Escape 鍵中斷繪製 (Escape Key Cancellation)**：支援按下鍵盤 `Escape` 鍵立即中斷並退出繪製模式，若有半完成之線段會自動從畫布及 store 中 pop 清除。同時需阻斷文字輸入框事件，避免在打字寫備註時誤觸。
