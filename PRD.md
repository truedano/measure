...
- **無縫複製 (Copy to Clipboard)**：
  - 提供「複製表格數據」按鈕。
  - **長度數值複製規格**：為方便使用者快速黏貼長度數據，點擊複製時**僅複製測量長度 (Length) 的數值清單**（例如 `77.12 mm` \n `77.11 mm`），每行一筆數值，不包含欄位標題、圖片名稱、線段 ID 或備註等其他資訊。
- **清除數據**：
...
4. **長度數據複製驗證**：點擊「Copy Data」按鈕，確認右上角有綠色 Toast 通知，打開 Excel 貼上，確認貼入的僅有 Length 長度數值清單，且每行一筆，不含其他欄位或標題。
5. **Vercel 構建與部署驗證**：專案透過 Vite 打包編譯無錯誤，且能夠在 Vercel 生產環境正常運作。

---

## 7. 新增擴充功能規格 (Enhanced Feature Specifications)

為了進一步提升操作體驗、支援海量測量數據的本機留存、以及改善多圖片下的組織結構，新增以下規格：

### 1. 順序性引導 UI 設計 (Wizard Sequence UI)
- **規格**：頂部工具列將參考線校正與測量線繪製拆分為清晰的步驟引導：
  - **Step 1 (校正比例)**：高亮標示 DPI 輸入區或「繪製參考線」按鈕。提示使用者必須先為當前圖片完成比例校正。
  - **Step 2 (新增測量)**：只有在 Step 1 完成（即 scale 已校正且不為 1）後，才高亮並啟用「Add Line」測量按鈕，引導使用者一步步操作。

### 2. 本地資料留存 (IndexedDB Storage)
- **規格**：引入本地端數據庫 `IndexedDB` 保存整個 Workspace 的狀態：
  - **儲存對象**：包含使用者建立的「資料夾/群組」、所有上傳的圖片資料（Base64 格式）、測量線段 (lines)、參考線定義 (referenceLine/referenceLength/unit/scale)、DPI 配置。
  - **持久化機制**：每當使用者載入新圖片、畫線、修改筆記、拖曳端點或變更資料夾時，系統自動將最新 Workspace 狀態非同步寫入 IndexedDB。
  - **復原載入**：網頁載入時，自動從 IndexedDB 復原上一次的完整 Workspace。

### 3. 左側圖片群組與資料夾管理 (Image Grouping & Folders)
- **規格**：在左側 Sidebar 新增「資料夾/群組」的邏輯分區：
  - 支持建立新資料夾、修改資料夾名稱、刪除資料夾。
  - 圖片卡片可以透過下拉選單或搬移操作，歸類到特定資料夾中。
  - 刪除資料夾時，提供自訂對話框詢問「是否一併刪除資料夾內的所有圖片」。
  - **資料夾內建上傳入口**：在每個自訂資料夾的 Header 右側提供一個小型的上傳按鈕。當使用者點擊特定資料夾的上傳按鈕匯入圖片時，系統會自動將新上傳的圖片歸屬於該資料夾下，免去手動移轉的重複操作。
  - **圖片順序調整 (Image Sorting)**：在資料夾內部的圖片卡片右側，提供「上移」與「下移」的按鈕（以微型 SVG 箭頭呈現）。使用者點擊按鈕，可在資料夾內部（或全域）調整圖片的前後順序。順序調整後，狀態會自動防抖寫入 IndexedDB 留存。

### 4. 畫布繪圖預覽與視覺優化 (Interactive Canvas Preview & Clarity)
- **規格**：優化 CanvasArea 與 Composables：
  - **動態線段預覽 (Interactive Preview)**：在點擊確定起點、尚未點擊終點的狀態下，滑鼠移動時必須在畫布上渲染一條**動態虛線（預覽線）**，並即時顯示當前拉伸長度，讓使用者清楚感受繪圖狀態。
  - **減輕視覺遮擋 (Visual Optimization)**：
    - 將端點 Handles 大小略微縮小（如 `8px`），並改為半透明發光邊框，減少對圖像中測量特徵的遮擋。
    - 調整數值標籤（Line Length Label）的絕對定位高度與層級，使其置於端點下方或側邊。測量線與**參考線皆支援在畫布上點擊 `x` 按鈕直接刪除**，以維持操作邏輯的一致性，刪除參考線會清空比例尺。

### 5. 數據試算表過濾篩選 (Data Sheet Folder Filter)
- **規格**：在 Measurement Data Sheet 的頂部控制列，新增「資料夾篩選」下拉選單：
  - 選項包含：「All Folders (全部顯示)」、「Uncategorized (僅顯示未分類圖片)」、以及各個自訂的資料夾名稱。
  - 選擇特定資料夾後，表格僅渲染該資料夾下圖片的測量線數據。
  - **複製聯動**：點擊「Copy Data」複製的長度數值列表，也必須同步過濾，僅複製當前篩選視圖中顯示的測量長度。
